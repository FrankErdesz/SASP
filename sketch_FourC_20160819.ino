...
unsigned long tempmillis, lasthomemillis, readingmillis;
...
void setup() {
  ...
  if (unitnumber == 21) {
    elevfactor = 885;//1150,1050
    elevintercept = 1020;//850,900
    homeangleoffset = 3.926;
    ElevSlow = 1;
  }
  if (unitnumber == 22) {
    elevfactor = 618;
...
void loop() {
  ...
  LastPOPMode = POPMode;
  readingmillis = millis();  // time point is start of short gate
  tempvalue = analogRead(temppin);
  homevalue = analogRead(homepin);
  runninghome = 0.33 * homevalue + 0.66 * runninghome;
  if ((runninghome > homethreshold) && (readingmillis > (lasthomemillis + 3000)))  // don't reset home value more than once a second
    lasthomemillis = readingmillis;
  ...
  if ((readcounter % (4 * ElevSlow)) == 2) {
    ...
    corrsintiltx = -sin(pitch * dtor); // or (plus) rrunningsintiltx, y
    corrsintilty = -sin(roll * dtor);
